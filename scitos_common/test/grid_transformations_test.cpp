...
TEST(DISABLED_Scale0_5XTest, WorldToGrid) {
  ...
  ASSERT_EQ(grid::world_to_grid(pos, origin, grid_size, 0.5f), expected);
}
...
TEST(NoScaleTest, GridToWorld) {
  ...
  auto res = grid::grid_to_world(pos, origin, 1.f);
...
TEST(Scale2XTest, GridToWorld) {
  ...
  auto res = grid::grid_to_world(pos, origin, 2.f);
  ASSERT_FLOAT_EQ(res.x, expected.x);
  ASSERT_FLOAT_EQ(res.y, expected.y);
}
...
TEST(Scale0_5XTest, GridToWorld) {
  ...
  auto res = grid::grid_to_world(pos, origin, 0.5f);
  ASSERT_FLOAT_EQ(res.x, expected.x);
  ASSERT_FLOAT_EQ(res.y, expected.y);
}
...
TEST(NoScaleTest, WorldToGridToWorld) {
  ...
  Vec2<float> grid_size = {8.f, 8.f};
  auto expected = pos + Vec2(0.5f, 0.5f);
  auto tmp = grid::world_to_grid(pos, origin, grid_size, 1.0);
  auto res = grid::grid_to_world(tmp.value(), origin, 1.0f);
  ASSERT_FLOAT_EQ(res.x, expected.x);
  ASSERT_FLOAT_EQ(res.y, expected.y);
}
...
TEST(Scale2XTest, WorldToGridToWorld) {
  ...
  auto tmp = grid::world_to_grid(pos, origin, grid_size, 2.0);
  auto res = grid::grid_to_world(tmp.value(), origin, 2.0f);
  ASSERT_FLOAT_EQ(res.x, expected.x);
  ASSERT_FLOAT_EQ(res.y, expected.y);
}
...
TEST(Scale0_5XTest, WorldToGridToWorld) {
  ...
  auto tmp = grid::world_to_grid(pos, origin, grid_size, 0.5f);
  auto res = grid::grid_to_world(tmp.value(), origin, 0.5f);
  ASSERT_FLOAT_EQ(res.x, expected.x);
  ASSERT_FLOAT_EQ(res.y, expected.y);
}
